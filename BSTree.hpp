...
void BSTree<DATATYPE, KEYTYPE>::addNode(KEYTYPE key, Node<DATATYPE, KEYTYPE> * leaf, DATATYPE& data) {
    ...
    if (key <= leaf->Key())
    {
        ...
        {
            Node<DATATYPE, KEYTYPE> *newleftPtr = new Node<DATATYPE, KEYTYPE>;
            newleftPtr->setKey(key);
            newleftPtr->setData(data);
            leaf->setLeft(newleftPtr);
            newleftPtr->setParent(leaf);
    ...
    else
    {
        ...
        {
            Node<DATATYPE, KEYTYPE> *newrightPtr = new Node<DATATYPE, KEYTYPE>;
            newrightPtr->setKey(key);
            newrightPtr->setData(data);
            leaf->setRight(newrightPtr);
            newrightPtr->setParent(leaf);
...
template <typename DATATYPE, typename KEYTYPE>
Node<DATATYPE, KEYTYPE> * BSTree<DATATYPE, KEYTYPE>::modifyNodeKey(KEYTYPE key, KEYTYPE modifiedKey)
{
    Node<DATATYPE,KEYTYPE> * findNodePtr = findNode(key);
    if (findNodePtr != nullptr)
    {
        findNodePtr->setKey(modifiedKey);
        DATATYPE a = findNodePtr->Data();
        a.name=modifiedKey;
        findNodePtr->setData(a);
        
        cout<< modifiedKey << " is changed." <<endl;
    }else{
...
template <typename DATATYPE, typename KEYTYPE>
void BSTree<DATATYPE, KEYTYPE>::getData(Node<DATATYPE, KEYTYPE> * node, vector<DATATYPE>& dataset)
{
    if (node == nullptr )
    {
    ...
    getData(node->Left(),  dataset);
    dataset.push_back(node->Data());
    getData(node->Right(), dataset);
}

template <typename DATATYPE, typename KEYTYPE>
void BSTree<DATATYPE, KEYTYPE>::getData(vector<DATATYPE>& dataset)
{
    
    getData(root, dataset);
}
